...
const scrollWrapper = document.querySelector("#TEST_noScrollOnBody"); /* 07/02/2022 */
...
if(getScrollTop() == 0) {
    ...
    scrollWrapper.style.overflow = "hidden"; // 07/02/2022
    ...
    tl_intro
        ...
        .add(() => { 
            ...
            scrollWrapper.style.overflow = "auto"; // 07/02/2022
...
let simulScrollVal = 0, pxPerWheel = 30, directionWheel = 0;
window.addEventListener('wheel', (e) => {   console.log(simulScrollVal); //TEST
    console.log("wheel!!", e.deltaY);

    if(e.deltaY > 0) {
        simulScrollVal += pxPerWheel;
        directionWheel = 1;
    } else if(e.deltaY < 0) {
        simulScrollVal -= pxPerWheel;
        directionWheel = -1;
    }
    scrollWrapper.scrollTo(0, simulScrollVal);
});

//window.addEventListener('scroll', () => {   /* 07/02/02022 - TEST no scroll on body !!!! */
scrollWrapper.addEventListener('scroll', () => {   /* 07/02/02022 - TEST no scroll on body !!!! */
...
function goToLabel() {  
    ...
    const direction = directionWheel;
    ...
    /* gsap.to(window, { */ // 07/02/02022 - TEST no scroll on body !!!!
    gsap.to(scrollWrapper, {  // 07/02/02022 - TEST no scroll on body !!!!
...
const st = {
    /* trigger: "body", */trigger: "#TEST_son", scroller: "#TEST_noScrollOnBody", /* TEST au 07/02/2022 */
    start: "top top",
    end: `bottom bottom`,
...
function getScrollTop() {
    document.querySelector("#titi").innerText = scrollWrapper.scrollTop; //TEST
    ...
    document.querySelectorAll(".projectCard").forEach(p => {
        ...
        if(navigation_projects[i].back == false) { 
            ...
            leftArrow.addEventListener("click", () => {
                 ...
                 */gsap.to(scrollWrapper, {duration: duree, scrollTo: {y: (ratio * parseFloat(totalTimePreviousLabel)), autoKill: true}, ease: easing, onComplete: () => { menuOrArrowClicked = false } }); 
            })
        }
        ...
        if(navigation_projects[i].forth == false) {
            ...
            rightArrow.addEventListener("click", () => {
                ...
                gsap.to(scrollWrapper, {duration: duree, scrollTo: {y: (ratio * parseFloat(totalTimeNextLabel)), autoKill: true}, ease: easing, onComplete: () => { menuOrArrowClicked = false } });
            })
        }
